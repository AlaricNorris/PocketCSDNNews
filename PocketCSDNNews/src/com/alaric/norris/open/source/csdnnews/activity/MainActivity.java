...
public class MainActivity extends BaseFragmentActivity {
	...
	private void initData() {
		fragmentPagerAdapter = new TabAdapter(getSupportFragmentManager(), PocketCSNDNewsApp.getContext().getResources().getStringArray(R.array.tabTitles));// 获取Tab的适配器
		...
		mode.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (mode.getText().equals(res.getString(R.string.night))) {
					PreferenceUtil.write(PocketCSNDNewsApp.getContext(), SharedPreferencesConstant.CONFIG, "theme", 0);
					ThemeControl.changeToTheme(MainActivity.this, ThemeControl.THEME_NIGHT);
				} else {
					PreferenceUtil.write(PocketCSNDNewsApp.getContext(), SharedPreferencesConstant.CONFIG, "theme", 1);
